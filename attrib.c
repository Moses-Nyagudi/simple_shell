...
/**
 * buffer_input - buffers connectored commands
    ...
    if (!*len) /* if nothing left in the buffer, fill it */
    {
        ...
        *buf = NULL;
        ...
        if (r > 0)
        {
            ...
            /* if (_strchr(*buf, ';')) is this a command connector? */
            {
                *len = r;
                info->cmd_buf = buf;
            }
...
{
    static char *buf; /* the ';' command connector buffer */
    ...
    if (r == -1) /* EOF */
        return -1;
    if (len)    /* we have commands left in the connector buffer */
    {
        j = i; /* init new iterator to current buf position */
        p = buf + i; /* get pointer for return */

        check_connector(info, buf, &j, i, len);
        while (j < len) /* iterate to semicolon or end */
        {
            if (is_connector(info, buf, &j))
                break;
            j++;
        }
    ...
    *buf_p = buf; /* else not a connector, pass back buffer from custom_getline() */
